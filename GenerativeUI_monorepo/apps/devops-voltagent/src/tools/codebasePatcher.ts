...
export const codebasePatcherTool = createTool({
  name: "patchCodebase",
  description: "Apply a fix or generate a test patch for an identified codebase gap.",
  parameters: z.object({
    filePath: z.string().describe("The file to patch or create"),
    content: z.string().describe("The new content or fix"),
    type: z.enum(["test", "source"]).describe("Whether this is a test fix or source code fix"),
  }),
