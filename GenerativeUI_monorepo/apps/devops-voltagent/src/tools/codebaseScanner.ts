...
export const codebaseScannerTool = createTool({
  name: "scanCodebase",
  description: "Scan the codebase for missing tests, lint errors, or structural gaps using built-in scripts and tools.",
  parameters: z.object({
    directory: z.string().describe("The root directory to scan"),
    scanType: z.enum(["test-coverage", "linting", "typecheck"]).describe("The type of scan to run"),
  }),
  execute: async ({ directory, scanType }) => {
    try {
      ...
      return {
        ...
        details: "Mock output: Needs test coverage for src/components/Button.tsx",
      };
    } catch (error: any) {
      return {
        status: "failed",
