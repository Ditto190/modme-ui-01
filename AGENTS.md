...
# Wasp Knowledge [GENERATED BY WASP PLUGIN SETUP — keep section title]

Wasp fullstack agent kit (skills under `.agents/skills/`): `wasp-plugin-init`, `start-dev-server`, `expert-advice`, `deploying-app`, `wasp-plugin-help`.

- Docs map: https://wasp.sh/llms.txt → versioned https://wasp.sh/llms-0.24.txt (fetch raw markdown guides from the map; prefer over doc MCP bloat)
- Knowledge file: [`docs/wasp/general-wasp-knowledge.md`](docs/wasp/general-wasp-knowledge.md)
- Browser visibility: `chrome-devtools` MCP in `.cursor/mcp.json`

# Serena MCP

Serena provides symbol-level IDE tools over MCP and **auto-starts with Cursor** via `.cursor/mcp.json` (`--context ide --project-from-cwd`).

- Ensure: `yarn serena:ensure` · Doctor: `yarn serena:doctor` · Skill: [`.agents/skills/serena/SKILL.md`](.agents/skills/serena/SKILL.md)
- Project config: `.serena/project.yml` · ADR: [`next-forge/docs/adr/0013-serena-mcp-startup.md`](next-forge/docs/adr/0013-serena-mcp-startup.md)
- Install (once): `uv tool install -p 3.13 serena-agent` then `serena init`
