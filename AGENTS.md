...
- For shared LSP across Cursor, VS Code, and worktrees, prefer instance-sharing multiplexers (`lspmux`) over feature-merging aggregators (`rassumfrassum`/Eglot-style).
...
- Remote `origin` (`modme-ui-01`) has unrelated histories: legacy `main` (GenUI Workbench layout) vs active `dev`/reconciliation line (Monorepo_ModMe); do not assume a shared merge base.
- `lspmux` keys language-server instances by LSP `initialize` workspace folders — Cursor + VS Code on the same checkout share one server; each worktree path gets its own instance under one daemon.
