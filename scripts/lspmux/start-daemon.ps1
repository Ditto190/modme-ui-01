...
if (-not (Test-Path $lspmux)) {
  ...
  Write-Host "Run: yarn lspmux:install" -ForegroundColor Yellow
  exit 1
}
...
# PowerShell cannot redirect stdout+stderr to the same file via Start-Process.
$proc = Start-Process -FilePath 'cmd.exe' -ArgumentList @(
  '/c',
  "`"$lspmux`" server >> `"$logFile`" 2>&1"
...
Write-Host "Or run: yarn lspmux:start:foreground" -ForegroundColor DarkGray
exit 1
