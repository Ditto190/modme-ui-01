...
if ($Json) {
  ...
  if ($installed) {
    ...
    if ($daemonOk) {
      ...
      Write-Host "daemon: not reachable - run .\scripts\lspmux\start-daemon.ps1" -ForegroundColor Yellow
    }
  }
  else {
    Write-Host $statusText -ForegroundColor Yellow
    Write-Host ""
    Write-Host "Install: .\scripts\lspmux\install.ps1" -ForegroundColor DarkGray
  }
}
