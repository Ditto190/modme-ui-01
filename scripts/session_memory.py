    ...
    def __init__(
        ...
        if mode == "http":
            self.client = chromadb.HttpClient(
                host=host or "localhost", port=port or 8001
            )
        ...
        print(
            f"✅ SessionMemory initialized (mode={mode}, session={self.session_id}, embedding_dim={embedding_dim})"
        )
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=8001, help="ChromaDB port (http mode)"
    )
    parser.add_argument(
        "--persist-dir", default=DEFAULT_PERSIST_DIR, help="Persistence directory"
    )
