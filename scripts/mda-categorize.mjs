...
import {
  matchCategoryWithDetails,
  printCategoryReport,
} from './lib/category-scorer.mjs';
...
async function runTaxonomyTeam() {
  ...
  for (const entry of entries) {
    ...
    const match = matchCategoryWithDetails(fullText, categories);
    ...
    if (DRY_RUN) {
      const catLabel = match.slug ?? 'uncategorized';
      console.log(
        `[dry-run] ${entry.id.slice(0, 8)}... → tags:${mergedTags.join(',')}, severity:${inferredSeverity}, category:${catLabel} (score:${match.score})`
      );
      if (match.slug) {
        samples.push({
          id: entry.title ?? entry.id.slice(0, 8),
          from: 'uncategorized',
          to: match.slug,
          score: match.score,
        });
  ...
  console.log(`\n[mda/taxonomy] Done — ${updated} entries categorized, ${failed} without category match`);
}
  ...
  for (const entry of recent) {
    ...
    for (const match of similar ?? []) {
      ...
      if (!DRY_RUN) {
        ...
        console.log(`[dry-run] relation: ${entry.id.slice(0, 8)} → ${match.id.slice(0, 8)} (${relationType}, ${match.similarity.toFixed(3)})`);
      }
    }
  }
