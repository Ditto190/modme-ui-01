...
| `API.CATALOGUE.LIST` | API | API | `next-forge/apps/api/app/catalogue/route.ts` | GET catalogue_items (list/popular/search) |
| `SKILLS.INDEX.GENERATE` | SKILLS | AGENT | `scripts/skills-index-generate.mjs` | Manifest from `.agents/skills` |
| `CATALOGUE.ITEM.SYNC` | CATALOGUE | AGENT | `scripts/catalogue-sync.mjs` | Promote schemas + skills index → registry |
...
# 4. Generate outputs + catalogue registry
yarn skills:index
node scripts/output-generate.mjs --type all
node scripts/catalogue-sync.mjs

# Or run full pipeline end-to-end (includes skills index + catalogue sync)
...
- [x] Skills catalogue loader — lazy `@skill-id` via `skill-loader.ts` + `yarn skills:index`
