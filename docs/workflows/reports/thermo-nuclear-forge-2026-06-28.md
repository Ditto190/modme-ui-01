...
3. **Suggestion:** `molecule-contract` CI job enabled on schema path changes
...
|-------|---------|--------|
| Molecule contract | `yarn molecule-index:verify` | PASS |
| WS golden | `bun test packages/schemas/ws-contract.test.ts` | PASS |
| Playwright E2E | `cd next-forge && bun run test:e2e` | Requires agent-server :8000 — run locally |
| Telemetry audit | `yarn telemetry:audit --lens all` | Optional post-merge |
...
- PORTING slices: [`docs/migration/porting-guide-slices.md`](../../migration/porting-guide-slices.md)
