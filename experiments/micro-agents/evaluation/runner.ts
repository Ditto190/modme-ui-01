...
async function runAgent(
  query: string
): Promise<{ output: string; tool_calls: string[]; duration_ms: number }> {
...
function evaluateResponse(
  ...
  if (expected.expected.command_contains) {
    metrics.command_contains = expected.expected.command_contains.every(
      (part) => response.output.toLowerCase().includes(part.toLowerCase())
    );
  }
...
async function main() {
  ...
  for (const response of responses) {
    const expected = expectedResponses.find(
      (e) => e.query_id === response.query_id
    );
    if (!expected) {
