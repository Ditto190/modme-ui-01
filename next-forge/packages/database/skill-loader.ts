...
export function loadSkillIndex(
  indexPath: string,
  rootKind: SkillMetaSource = "project"
...
export function createSkillResolverFromEnv(cwd = process.cwd()): SkillResolver {
  ...
  return createSkillResolver({
    ...
    maxSkillsPerTurn: Number.parseInt(
      process.env.MODME_MAX_SKILLS_PER_TURN ?? "8",
      10
    ),
    overflowBehavior,
  });
...
export async function loadSkillBodies(
  ...
  for (const meta of metas) {
    ...
    if (!skillFileStat.isFile() || skillFileStat.isSymbolicLink()) {
      throw new Error("SKILL.md must be a regular file inside the skills root");
    }
...
export async function loadSkillBodiesHybrid(
  ...
  for (const meta of metas) {
    const root =
      meta.rootKind === "global" && globalSkillsRoot
        ? globalSkillsRoot
        : projectSkillsRoot;
