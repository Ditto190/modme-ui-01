...
function makeTempSkillsLayout(options?: {
  ...
  if (options?.withEscapePath) {
    ...
    fs.writeFileSync(path.join(root, "skills_index.json"), JSON.stringify(index), "utf8");
...
describe("collectReferencedSkillIds", () => {
  ...
  it("supports nested ids with slashes", () => {
    ...
    const messages: Message[] = [{ role: "user", content: "Ask @agent-squad/mason" }];
    expect(collectReferencedSkillIds(messages, index)).toEqual(["agent-squad/mason"]);
  });
});
...
describe("loadSkillBodies security", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const { skillsRoot, root } = makeTempSkillsLayout({ withSymlinkEscape: true });
      const index = loadSkillIndex(path.join(root, "skills_index.json"));
